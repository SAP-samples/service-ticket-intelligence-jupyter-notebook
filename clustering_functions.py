...
#     url = connection.uaa_url + "/oauth/token"
#     headers = {
#         'Content-Type': "application/x-www-form-urlencoded"}
#     body = {u'grant_type': u'client_credentials',
#             u'response_type': u'token',
#             u'client_id': connection.client_id,
#             u'client_secret': connection.client_secret
#             }
...
class STIFunctions:
    ...
    def clustering_plot_treemap(self,clusters, lang, label="keywords", top_k=50):
        cluster = clusters[lang]["clusters"]
        labels = []
        lenth = []
        conf = []
        for cls in cluster:
            labels.append("\n ".join([kyw for kyw, _ in cls["keywords"][:3]]))
            lenth.append(len(cls["tickets"]))
            conf.append(cls["conf"])

        cmap = matplotlib.cm.Blues
        conf = np.array(conf)
        conf = (conf - min(conf)) / max(conf - min(conf)) * 0.7 + 0.2
        mini = min(conf)
        maxi = max(conf)
        norm = matplotlib.colors.Normalize(vmin=mini, vmax=maxi)
        colors = [cmap(norm(value)) for value in conf]

        squarify.plot(sizes=lenth[:top_k], label=labels[:top_k], alpha=.6, color=colors)
        plt.axis('off')
        plt.gcf().set_size_inches(18, 8)
        plt.show()

    def clustering_plot_circlepacking(self,clusters, lang, label="keywords", top_k=50):

        def _bubbles(circles, conf, labels=None, lim=None):
            labels = [circ.get_default_label(i, c) for i, c in enumerate(circles)]

            """Debugging function displays circles with matplotlib."""
            if not labels:
                labels = range(len(circles))
            _, ax = plt.subplots(figsize=(16.0, 16.0))

            N = len(circles)
            col = 1 - 1. / N * np.arange(0, N)
            ...
            for circle, label, k, c in zip(circles, labels, col, colors):
                x, y, r = circle
                ax.add_patch(plt.Circle((x, y), r, alpha=0.9, linewidth=2, fill=False, color=c))
                ax.text(x, y, label)
            enclosure = circ.enclose(circles)
            n = len(circles)
            if enclosure in circles:
                n = n - 1
            d = circ.density(circles, enclosure)
        ...
        circ_data = [{"id": kyw, "datum": le} for kyw, le in zip(labels[:top_k], lenth[:top_k])]
        circles = circ.circlify(data=circ_data, show_enclosure=True)
        _bubbles(circles, conf)
    ...
    @add_headers(True)
    @parse_response(True)
    def clustering(self,data_payload, headers):
        url = f"{self.connection.service_url}/sti/text/cluster"
        response = requests.post(url, headers=headers, data=json.dumps(data_payload))
